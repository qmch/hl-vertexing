...
void efficiencyPlot(){
  ...
  //TTreeReaderValue<float> tempWeight = {fReader, "weight"};
  ...
  while (fReader.Next()) {
    ...
    */
    ...
    my_histo->Fill(recovx_z[0]);
    /*
    if (abs(truthvx_z[0]-recovx_z[0])<1){
      my_histo2->Fill(recovx_z[0]);
    }
    */
  ...
  */
  ...
  my_histo2->GetXaxis()->SetTitle("Z-position");
  ...
  //c1->Print("plots/output_efficiencyPlot_firstfewselection_ratio.pdf");
